...
/**
 * En esta clase se visualizan la informacion que la clase Excel no pudo reconocer
 * @author Pablo
 *  
 */
public class Conflictos extends javax.swing.JFrame {
    ...
    private void GuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_GuardarActionPerformed
        dispose();
        try {
            //Ventana de seleccionar donde se va a guardar el excel generado  
    ...
    /**
     * Oyende del mouse para reconocer cuando el usuario le da la "Boton" de resolver conflicto, aqui es donde se le da la funcionalidad a esa parte
     * @param evt 
     * 
     * 
     */
    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
//        Saca que fila y que columna esta el usuario dio click              
        int fila=jTable1.rowAtPoint(evt.getPoint());
        int columna=jTable1.columnAtPoint(evt.getPoint());
        //Verifica si esta dentro de los margenes de la tabla
        if(fila < jTable1.getRowCount() && fila >=0 && columna < jTable1.getColumnCount() && columna>=0){
           // Si es le dan click a la columna donde esta los botones ,abre la ventana de resolver conflicto de la finformacio presentada en esa fila 
            if(columna==4){
                int hoja=0;
//              Saca que tipo de conflicto es  y localiza a que hoja del Excel generado corresponde ,si es de AutoresTEC,Autores Externos , o de Universidad no reconocida 
                if(jTable1.getValueAt(fila, 3)=="AutoresTEC"){
                    hoja=0;
                }
                 if(jTable1.getValueAt(fila, 3)=="Autores Externos"){
                    hoja=1;
                }
                if(jTable1.getValueAt(fila, 3)=="Univesidad no reconocida"){
                    hoja=2;
                }
                //Para los combobox de escuelas y campus
                if("Escuela o Unidad".equals((String) jTable1.getValueAt(fila, 2))||"Campus".equals((String) jTable1.getValueAt(fila, 2))){
                    if("Escuela o Unidad".equals((String) jTable1.getValueAt(fila, 2))){
                        ...
                        //Poner toda el texto de donde no reconocio la informacion
                        ...
                        //Abrir la ventana de resolver Conflicto
